...
test('manual.html: structure (Part I reference + Part II cookbook 21 + Part III algorithms 10, links)', async ({ page }) => {
  await page.goto(DOC);
  ...
  expect(await page.locator('.ex').count()).toBe(31);          // 21 cookbook + 10 algorithm cards (v24: +E9 quantum counting)
  expect(await page.locator('article.algo').count()).toBe(10); // Part III (v24: E1–E10)
  ...
  for (const id of ['E1','E2','E3','E4','E5','E6','E7','E8','E9','E10']) await expect(page.locator(`#ex-${id}`)).toBeVisible();
