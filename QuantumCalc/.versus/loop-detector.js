...
function main() {
  ...
  process.stdin.on("end", () => {
    ...
    try {
      ...
      const result = engine.incrementLoopCounter(command);
      if (result.blocked) {
        deny(
          `[Versus] Safeguard S6 activated: test command executed ${result.count}x. Loop detected. STOP and:
1. Check if the bug has a known solution (Tier 2 \u2014 look for a reference implementation)
2. Consult specs/technical/ and specs/examples/
3. If not found, reconsider the approach
4. Use record_decision() to document what you discovered
The counter will be reset when advancing to the next phase.`
        );
      } else {
        allow();
      }
